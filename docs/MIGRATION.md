...
Visual-test mode exists only behind `?visualTest=1`. Pixi tickers are frozen, Party creature sprites are pinned to a deterministic frame, and the guarded scene-loader explicitly renders after loading an interior so screenshots cannot inherit the previous frame. Normal runtime animation and behavior are unchanged.
