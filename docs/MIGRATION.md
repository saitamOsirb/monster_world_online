...
visual-tests/
  game.visual.spec.ts             deterministic Town/menu/party pixel hashes
```
...
3. `pnpm test:visual` — launches the game in deterministic visual-test mode and compares SHA-256 hashes of the 240×160 canvas for Town, the menu and Party Screen.
4. `pnpm build` — validates strict TypeScript and the Vite production bundle.

Visual-test mode exists only behind `?visualTest=1`; it freezes Pixi tickers so animated water and UI sprites cannot make screenshots nondeterministic. Normal runtime behavior is unchanged.
...
2. Expand visual fixtures to each interior (`OaksLab`, `PlayerHomeFloor1`, `RivalHomeFloor`) and any future imported map.
